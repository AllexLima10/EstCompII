...
q1$b <- list(n_1000 <- list(Probabilidade=1-integral_Simpson(f1,0,2,1000,funcao_real=f1_real)$integral,erro=integral_Simpson(f1,0,2,1000,funcao_real=f1_real)$erro),n_10000=list(Probabilidade=1-integral_Simpson(f1,0,2,10000,funcao_real=f1_real)$integral,erro=integral_Simpson(f1,0,2,10000,funcao_real=f1_real)$erro))
q1$b
...
q1$c <- list(n_1000 <- list(Probabilidade=1-integral_Monte_Carlo(f1,0,2,1000,f1_real)$Integral_Estimada,erro=integral_Monte_Carlo(f1,0,2,1000,f1_real)$Erro,Desvio_padrao_estimado=sqrt(integral_Monte_Carlo(f1,0,2,1000,f1_real)$Variancia_Estimada)
),n_10000=list(Probabilidade=1-integral_Monte_Carlo(f1,0,2,10000,f1_real)$Integral_Estimada,erro=integral_Monte_Carlo(f1,0,2,10000,f1_real)$Erro,Desvio_padrao_estimado=sqrt(integral_Monte_Carlo(f1,0,2,10000,f1_real)$Variancia_Estimada)
))
q1$c

#d)
set.seed(10)
funcao_imp_1d <- function(x){ifelse(x>2,2/x^2,0)}
funcao_inv_1d <- function(x){ifelse(x<1/2,-2/x,1)} #função calculada no caderno (escreva a equação de y e tente encontrar uma equação para x)
